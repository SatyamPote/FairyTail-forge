...
export const StoryGeneratorModal = ({
  isOpen,
  onClose,
}: {
  isOpen: boolean;
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const storyData = JSON.parse(fullResponse) as {
        title?: string;
      ...
      const panelsList = (storyData.panels || []).map((p) => ({
        ...p,
      ...
      if (message.includes('system memory')) {
        alert(
          `Ollama Memory Error: Your system doesn't have enough RAM for ${selectedModel}. \n\nPlease select a smaller model like 'qwen2.5:3b' or 'tinyllama'.`
        );
      } else {
        alert(message || 'Generation failed. Make sure Ollama is running.');
      }
    } finally {
            ...
            {isGeneratingStory ? (
              <div className="flex flex-col items-end gap-1">
                ...
                <span className="text-[10px] text-slate-500 italic">
                  This can take 2-4 mins on some hardware
                </span>
              </div>
            ) : (
              <>
